...
public class ADB {
    ...
    public static void tapToBackGround(){ command("adb shell input keyevent KEYCODE_APP_SWITCH"); }

    public static void killThisApp(String cPackage){
        command("adb shell 'pm clear '"+cPackage);
    }
    ...
    public String getForegroundActivity(){
        return command("adb shell dumpsys window windows | grep mCurrentFocus");
    }
    ...
    public void redirectToHome(){
        try {
            pr = rt.exec("adb shell input input keyevent 3");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public void inputText(String value){
        try {
            pr = rt.exec("adb shell input text "+value);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public ArrayList getLogcatProcesses(){
        String[] output = command("adb shell top -n 1 | grep -i 'logcat'").split("\n");
