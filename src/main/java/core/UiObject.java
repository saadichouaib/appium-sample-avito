...
public class UiObject {
    ...
    public UiObject typeText(String value){
        if(isXpath()){
            Android.driver.getKeyboard().sendKeys(value);
            //Android.driver.findElementByXPath(locator).click();
            //Android.adb.inputText(value);
        }
        else {
    ...
    public UiObject typeTextAC(String value){
        if(isXpath()){
            Android.driver.getKeyboard().sendKeys(value);
        }
        else {
    ...
    public static void tapAutoCompleteSelection(UiObject uiObject){
        try {
            Thread.sleep(1000);
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
        int height = uiObject.getHeight();
        int width = uiObject.getWidth();

        Point cityFdLocation = uiObject.getLocation();
        int midX = (cityFdLocation.getX() + (width / 2));
        int midY = (cityFdLocation.getY() + (height / 2))+height;

        TouchAction action = new TouchAction(Android.driver);
        action.tap(midX,midY).perform();
    }
    ...
    public UiObject scrollTo(){
        ...
        if(isXpath()){
            ...
            Android.driver.findElementByAndroidUIAutomator(String.format("new UiScrollable(new UiSelector().scrollable(true).instance(0)).scrollIntoView("+ locator.substring(locator.indexOf("@text=\""), locator.indexOf("\"]")).replace("@text=\"", "") +")"));
        }
        else{
            String text;
            ...
            Android.driver.findElementByAndroidUIAutomator(String.format("new UiScrollable(new UiSelector().scrollable(true).instance(0)).scrollIntoView("+ locator +")"));
